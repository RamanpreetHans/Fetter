...
def userAccountInfo(request):
    ...
    n3 = User_Interest.objects.filter(user_name__username__icontains = request.user.username)
    n4 = User_Table.objects.filter(user_name__username__icontains = request.user.username)
    return render(request, 'basicapp/UserProfilePage.html', {'internships': n1, 'projects': n2, 'interests': n3, 'User': n4})
